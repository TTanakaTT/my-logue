...
- iPhoneのsfariで開くと、localeがenになってしまっている。safariでもlocaleの判断が正しく行われるようにしてください。
- ヘッダーの部分に言語選択ドロップダウンを追加してください。
- キャラクター詳細モーダルはモーダル外をクリックしたら閉じるようにしてください。
- ゲームの進行に関する情報はすべてグローバルにステート管理してください。コンポーネント間で props ドリルダウンしないようにしてください。
- 詳細モーダルに最大HPを表示してください。
- 敵の攻撃の時に、エフェクトが出ないので、修正してください。
- アクションに攻撃などの種類を追加し、特定の種類に対して強化や弱体化を行うアクションを追加してください。

...
<header class="bg-panel rounded-lg mb-4 p-4 flex flex-wrap items-center gap-4 text-sm text-gray-200">
...
<main>
  ...
  <button class="btn-base" on:click={() => { state.stepIndex +=1; nextProgress(state); }}>スキップ(デバッグ)</button>
    </section>
  {/if}
  ...
  {#if state.phase === 'combat'}
    <section class="bg-panel rounded-lg mb-4 p-4">
      ...
      {#if state.enemy}
        <p class="mb-2 text-sm">敵: {state.enemy.kind} HP {state.enemy.hp}/{state.enemy.baseHP} 攻撃 {state.enemy.attack + (state.enemy.buffAttack||0)}</p>
      {/if}
      <div class="mb-2 text-sm">行動 {state.actionUseCount}/2</div>
      <div class="flex flex-wrap gap-2">
        {#each state.actionOffer as id}
          {#if getAction(id)}
            <button class="btn-base" disabled={state.actionUseCount>=2} on:click={() => combatAction(state, id)}>
              {getAction(id)?.name}
            </button>
          {/if}
        {/each}
  ...
  <button class="btn-base" on:click={() => { state.stepIndex +=1; nextProgress(state); }}>進む</button>
    </section>
  {/if}
  ...
  {#if state.phase === 'victory'}
    <section class="bg-panel rounded-lg mb-4 p-4"><h2 class="mt-0 text-lg font-semibold">勝利!</h2></section>
  {/if}
  {#if state.phase === 'gameover'}
    <section class="bg-panel rounded-lg mb-4 p-4"><h2 class="mt-0 text-lg font-semibold text-bad">ゲームオーバー</h2></section>
  {/if}
  {#if state.phase === 'reward'}
    <section class="bg-panel rounded-lg mb-4 p-4">
      <h2 class="mt-0 text-lg font-semibold mb-2">{state.rewardIsBoss ? (state.rewardIsFinalBoss ? '最終ボス報酬' : 'ボス報酬') : '成長報酬'}</h2>
      <div class="flex flex-wrap gap-2">
        {#each state.rewardOptions || [] as r}
          <button class="btn-base" on:click={() => import('./game/state').then(m => m.pickReward(state, r.id))}>{r.label}</button>
        {/each}
      </div>
    </section>
  {/if}
    ...
    <div class="text-[0.85rem] leading-tight max-h-[200px] overflow-auto bg-logbg p-2 rounded-md">
      {#each state.log as entry}
        <div class={`mb-[2px] last:mb-0 flex items-start kind-${entry.kind}`}>
          <span class="inline-block min-w-[55px] text-center font-semibold text-[0.6rem] tracking-wide mr-1 px-1 py-[2px] rounded bg-gray-700 text-gray-300">{entry.kind}</span> {entry.message}
        </div>
      {/each}
    </div>
  </section>
...
<footer class="bg-panel rounded-lg mb-4 p-4">
